...
def numsorted(fns):
	prefix = fns[0].rsplit('/', 1)[0]
	return [prefix + '/' + str(i) + '.wav' for i in sorted(int(fn.rsplit('/', 1)[1].split('.', 1)[0]) for fn in fns)]

fns = reduce(lambda a, x: a + x, [numsorted(glob('%s/*' % base)) for base in sys.argv[2:]], [])
